...
- `config/` — wiring and settings. Env loading, the DI/composition root that binds concretes to interfaces, app settings, feature flags. Top-level — independent and **not** part of transport.
...
- `api/` — the API client: domain-meaningful calls that map to protocol details.
...
- **Transport seam (the wire, under controllers and clients).** Both sit on a transport interface, not a concrete protocol. Switch HTTP→WebSocket, or add a new RPC implementation and **zero changes** to controllers, clients, services, or models.
...
backend/
  ...
  shared/         # leaf utilities that import nothing else in the app
...
- **D — Dependency Inversion.** High-level policy depends on abstractions; details too. *Test:* does a service `new` a concrete class or reach for a global singleton? *Fix:* depend on an interface, inject the concrete, wire concretes at one composition root.

**Guardrail:** an interface earns its place only with a real second implementation, a genuine test seam, or a true policy/detail boundary. Don't invert dependencies where coupling rarely varies.
...
- **God unit** — a class/component owning lifecycle + data mapping + rendering + I/O + pagination. Extract one shared utility.
